...
class Rest_berita extends MY_Controller {

	function index() {
		$query = $this->vw_berita_m->get();
	...
	function save() {
       	...
       	if (empty($img)) {
        	...
        	if (!empty($current_path) && !empty($current_img)) {
       		...
       		if ($img !== 'noimg-post.jpg' && !empty($current_path) && !empty($current_img)) {
        		# code...
        		$path = FCPATH .'assets/images/news/'.$current_img;
	    		if (file_exists($path)){
	    			chmod($path, 0777);
				    unlink($path);
				}
        		$_pathImg = 'assets/images/news/';
        	} else {
        		$_pathImg = 'assets/images/news/';
        	}
       	}
		...
		echo json_encode(array('success' => $success, 'base_url' => base_url('cms-admin/berita')), 200);
	}
...
/* End of file rest_berita.php */
/* Location: ./application/modules/api/controllers/rest_berita.php */