                            ...
                            <div class="col-lg-5 col-md-5 col-sm-5 text-left">
                                 <div class="body pb-0">
                                    <div class="clearfix">
                                        ...
                                        <a href="<?php echo base_url('cms-admin/informasi/create') ?>" class="btn btn-md btn-outline-primary">
                                            <i class="fa fa-plus m-r-10"></i>Tambah Informasi
                                        </a>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    var table = $('.dataTable').DataTable({
        "ajax": {
            "url": "<?php echo base_url("api/rest_informasi") ?>",
            "type": "GET"
        },
        searching: true,
        paging: true,
        ...
        columns: [
            ...
            },
            {
                ...
                targets     : 7,
                render  : function (data, type, full, meta){
                    return '<a href="<?php echo base_url('cms-admin/informasi/update');?>/'+data.page_id+'" title="Update" class="btn btn-primary btn-xs" style="margin-right: 5px !important;"><i class="icon-note"></i></a>';
                }
            },
            {
                data        : "create_on",
    ...
    $('.dataTable tbody').on('change', 'tr[role="row"] td #publish', function (event, state) {
        ...
        $.ajax({
            url        : '<?php echo base_url('api/rest_informasi/publish') ?>',
            type       :'POST',
    ...
    $('.dataTable tbody').on("click", "tr[role='row'] td #delete", function (event) {
        ...
        swal({
            ...
            if (isConfirm) {
                setTimeout(function () {
                    $.ajax({
                        url        : '<?php echo base_url("api/rest_informasi/delete") ?>',
                        type       : 'POST',
    ...
    $('a#batch_delete').on('click', function(event) {
        ...
        swal({
            ...
            if (isConfirm) {
                setTimeout(function () {
                    $.ajax({
                        url        : '<?php echo base_url("api/rest_informasi/batch_delete") ?>',
                        type       : 'POST',
    ...
    $('a#batch_unpublish').on('click', function(event) {
        ...
        $.ajax({
            url        : '<?php echo base_url('api/rest_informasi/batch_publish') ?>',
            type       :'POST',
            data        : {'page_id': page_id},
            success: function(data) {
                // body...
    ...
    $('a#export_item_all').on('click', function(event) {
        event.preventDefault();
        /* Act on the event */
        location.href = '<?php echo base_url('api/rest_informasi/export_xls') ?>';
    });
</script>