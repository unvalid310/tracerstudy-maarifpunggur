                    ...
                    <div class="card Sales_Overview">
                        ...
                        <div class="row">
                            <div class="col-lg-5 col-md-5 col-sm-5 text-left">       
                                 <div class="body">
                                    <div class="clearfix">
                                        <a class="btn btn-md btn-outline-primary" href="<?php echo base_url("cms-admin/alumni/create") ?>"><i class="fa fa-plus m-r-10"></i>Tambah Data Alumni</a>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="collapse" id="collapseFilter">    
                            <div class="body pt-0">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 p-t-20 text-right" style="font-size: small;">
...
<script type="text/javascript">
    ...
    $('#name_category').multiselect({
        ...
        onChange: function(option, checked, select){
            ...
            if (checked) {
                ...
                table.column(13).search(option.val()).draw();
                $('.multiselect_div .multiselect-container').removeClass('show');
            }else{
                category_value = '';
                table.column(13).search(category_value).draw();
                $('.multiselect_div .multiselect-container').removeClass('show');
            }
    ...
    var table = $('.dataTable').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                targets     : 8,
                render  : function (data, type, full, meta){
                    return '<button type="button" id="delete" title="Delete" class="btn btn-danger btn-xs" style="margin-right: 5px !important;"><i class="icon-trash"></i></button><a href="<?php echo base_url('cms-admin/alumni/update/') ?>'+data.user_id+'" title="Update" class="btn btn-primary btn-xs"><i class="icon-note"></i></a>';
                }
            },
            {
                data        : "create_on",
    ...
    function format ( d ) {
        ...
        html = '<table id="detail" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;" width="100%">'
                    ...
                    +'</tr>'
                    +'<tr>'
                        +'<td class="header text-left" colspan="6" width="100%">Alumni</td>'
                    +'</tr>'
                    +'<tr>'
                        +'<td class="content text-left" width="10%"><b>Jurusan</b></td>'
                        +'<td class="content text-center" width="1%"><b>:</b></td>'
                        +'<td class="content text-left" width="20%" colspan="6">'+nama_jurusan+'</td>'
                    +'</tr>'
                    +'<tr>'
                        +'<td class="content text-left" width="10%"><b>Tahun Masuk</b></td>'
                        +'<td class="content text-center" width="1%"><b>:</b></td>'
                        +'<td class="content text-left" width="20%">'+th_masuk+'</td>'
                        +'<td class="content text-left" width="10%"><b>Tahun Ajar</b></td>'
                        +'<td class="content text-center" width="1%"><b>:</b></td>'
                        +'<td class="content text-left" width="20%">'+ta_masuk+'</td>'
                    +'</tr>'
                    +'<tr>'
                        +'<td class="content text-left" width="10%"><b>Tahun Lulus</b></td>'
                        +'<td class="content text-center" width="1%"><b>:</b></td>'
                        +'<td class="content text-left" width="20%">'+th_keluar+'</td>'
                        +'<td class="content text-left" width="10%"><b>Tahun Ajar</b></td>'
                        +'<td class="content text-center" width="1%"><b>:</b></td>'
                        +'<td class="content text-left" width="20%">'+ta_keluar+'</td>'
                    +'</tr>'
              +'</table>';
    ...
    $('.dataTable tbody').on('change', 'tr[role="row"] td #publish', function (event, state) {
        ...
        $.ajax({
            url        : '<?php echo base_url('api/rest_alumni/publish') ?>',
            type       :'POST',
    ...
    $('.dataTable tbody').on("click", "tr[role='row'] td #delete", function (event) {
        ...
        swal({
            ...
            if (isConfirm) {
                setTimeout(function () {
                    $.ajax({
                        url        : '<?php echo base_url("api/rest_alumni/delete") ?>',
                        type       : 'POST',
    ...
    $('#th_masuk').change(function () {
        table.column(10).search($('#th_masuk').val()).draw();
    });

    $('#th_keluar').change(function () {
        table.column(11).search($('#th_keluar').val()).draw();
    });
    ...
    function clearFilter() {
        ...
        $('#min, #max, #th_masuk, #th_keluar').val('').datepicker('update');
    ...
    $('a#batch_delete').on('click', function(event) {
        ...
        swal({
            ...
            if (isConfirm) {
                setTimeout(function () {
                    $.ajax({
                        url        : '<?php echo base_url("api/rest_alumni/batch_delete") ?>',
                        type       : 'POST',
    ...
    $('a#batch_unpublish').on('click', function(event) {
        ...
        $.ajax({
            url        : '<?php echo base_url('api/rest_alumni/batch_publish') ?>',
            type       :'POST',
            data        : {'user_id': user_id},
            success: function(data) {
                // body...
    ...
    $('a#export_item_all').on('click', function(event) {
        event.preventDefault();
        /* Act on the event */
        location.href = '<?php echo base_url('api/rest_alumni/export_xls') ?>';
    });
</script>